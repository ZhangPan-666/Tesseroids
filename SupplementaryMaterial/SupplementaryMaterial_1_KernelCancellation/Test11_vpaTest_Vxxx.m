...
KernelVxxx1=-cosd(FaiI).*cos(Alpha).*csc(Phi).^3.*(...
    cos(Alpha).^2.*8.*...
...
KernelVxxx2=vpa(-cosd(FaiI).*cos(Alpha).*csc(Phi).^3.*(...
    cos(Alpha).^2.*8.*...
    ((1-R2.*cos(Phi)).*l1pow5-(1-R1.*cos(Phi)).*l2pow5)./(l2pow5.*l1pow5)-...
    cos(Alpha).^2.*32.*cos(Phi).*...
    (R2.*(1-R2.*cos(Phi)).*l1pow5-R1.*(1-R1.*cos(Phi)).*l2pow5)./(l2pow5.*l1pow5)+...
...
KernelVxxx2=double(KernelVxxx2);

%%

figure(1);

PP1=plot(Longitude,KernelVxxx1);
PP1.LineWidth=2.0;
PP1.Color=[1,0,0];
PP1.ColorMode='manual';
hold on;

PP1=plot(Longitude,KernelVxxx2);
PP1.LineWidth=2.0;
PP1.Color=[0,1,0];
PP1.ColorMode='manual';
hold off;
...
AP=gca;
AP.YLim=[-1.1*max(abs(KernelVxxx2)),1.1*max(abs(KernelVxxx2))];

legend('Double-precision computation','Symbolic computation','Location','southeast')

figure(2);

PP1=plot(Longitude,KernelVxxx1-KernelVxxx2);
PP1.LineWidth=2.0;
PP1.Color=[0,0,1];
PP1.ColorMode='manual';
hold off;
