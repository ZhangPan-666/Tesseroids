...
KernelVzzy1=3.*cosd(FaiI).*Ty.*(R2.^4.*(1-R2.*cos(Phi)).*l1pow5-R1.^4.*(1-R1.*cos(Phi)).*l2pow5)./(l2pow5.*l1pow5);
...
Tx=vpa(cosd(FaiI).*sind(LamdaI-lamda),GuardDigits);
...
KernelVzzy2=vpa(3.*cosd(FaiI).*Ty.*(R2.^4.*(1-R2.*cos(Phi)).*l1pow5-R1.^4.*(1-R1.*cos(Phi)).*l2pow5)./(l2pow5.*l1pow5), ...
    GuardDigits);

KernelVzzy2=double(KernelVzzy2);

%%

figure(1);

PP1=plot(Longitude,KernelVzzy1);
PP1.LineWidth=2.0;
PP1.Color=[1,0,0];
PP1.ColorMode='manual';
hold on;

PP1=plot(Longitude,KernelVzzy2);
PP1.LineWidth=2.0;
PP1.Color=[0,1,0];
PP1.ColorMode='manual';
hold off;
...
AP=gca;
AP.YLim=[-1.1*max(abs(KernelVzzy2)),1.1*max(abs(KernelVzzy2))];

legend('Double-precision computation','Symbolic computation','Location','southeast')

figure(2);

PP1=plot(Longitude,KernelVzzy1-KernelVzzy2);
PP1.LineWidth=2.0;
PP1.Color=[0,0,1];
PP1.ColorMode='manual';
hold off;
