...
KernelVzy1=cosd(FaiI).*(...
    ...
    3.*cos(Phi).*Ty.*log((cos(Phi)-R2+l2pow1)./(cos(Phi)-R1+l1pow1)));
...
KernelVzy2=double(KernelVzy2);

%%

figure(1);

PP1=plot(Longitude,KernelVzy1);
PP1.LineWidth=2.0;
PP1.Color=[1,0,0];
PP1.ColorMode='manual';
hold on;

PP1=plot(Longitude,KernelVzy2);
PP1.LineWidth=2.0;
PP1.Color=[0,1,0];
PP1.ColorMode='manual';
hold off;
...
AP=gca;
AP.YLim=[-1.1*max(abs(KernelVzy2)),1.1*max(abs(KernelVzy2))];

legend('Double-precision computation','Symbolic computation','Location','northeast')

figure(2);

PP1=plot(Longitude,KernelVzy1-KernelVzy2);
PP1.LineWidth=2.0;
PP1.Color=[0,0,1];
PP1.ColorMode='manual';
hold off;
