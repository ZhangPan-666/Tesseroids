...
KernelVxyz1=-3.*cosd(FaiI).*Tx.*Ty.*(R2.^5.*l1pow5-R1.^5.*l2pow5)./(l2pow5.*l1pow5);
...
KernelVxyz2=vpa(-3.*cosd(FaiI).*Tx.*Ty.*(R2.^5.*l1pow5-R1.^5.*l2pow5)./(l2pow5.*l1pow5), ...
    GuardDigits);

KernelVxyz2=double(KernelVxyz2);

%%

figure(1);

PP1=plot(Longitude,KernelVxyz1);
PP1.LineWidth=2.0;
PP1.Color=[1,0,0];
PP1.ColorMode='manual';
hold on;

PP1=plot(Longitude,KernelVxyz2);
PP1.LineWidth=2.0;
PP1.Color=[0,1,0];
PP1.ColorMode='manual';
hold off;
...
AP=gca;
AP.YLim=[-1.1*max(abs(KernelVxyz2)),1.1*max(abs(KernelVxyz2))];

legend('Double-precision computation','Symbolic computation','Location','southeast')

figure(2);

PP1=plot(Longitude,KernelVxyz1-KernelVxyz2);
PP1.LineWidth=2.0;
PP1.Color=[0,0,1];
PP1.ColorMode='manual';
hold off;
