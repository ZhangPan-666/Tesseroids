...
KernelVxx1=cosd(FaiI).*(...
    cos(Alpha).^2.*csc(Phi).^2.*(...
    ...
    (1-3.*Tx.^2).*log((cos(Phi)-R2+l2pow1)./(cos(Phi)-R1+l1pow1)));
...
KernelVxx2=double(KernelVxx2);

%%

figure(1);

PP1=plot(Longitude,KernelVxx1);
PP1.LineWidth=2.0;
PP1.Color=[1,0,0];
PP1.ColorMode='manual';
hold on;

PP1=plot(Longitude,KernelVxx2);
PP1.LineWidth=2.0;
PP1.Color=[0,1,0];
PP1.ColorMode='manual';
...
AP=gca;
AP.YLim=[-1.1*max(abs(KernelVxx2)),1.1*max(abs(KernelVxx2))];

legend('Double-precision computation',...
    'Symbolic computation','Location','northeast')

figure(2);

PP1=plot(Longitude,KernelVxx1-KernelVxx2);
PP1.LineWidth=2.0;
PP1.Color=[0,0,1];
PP1.ColorMode='manual';
hold off;
