...
KernelVx2=vpa(cosd(FaiI).*cos(Alpha).*(...
    0.5.*csc(Phi).*(1-3.*cos(2.*Phi)).*((l1pow1-l2pow1)./(l2pow1.*l1pow1))+...
    0.5.*(-cot(Phi)+3.*csc(Phi).*cos(3.*Phi)).*((R2.*l1pow1-R1.*l2pow1)./(l2pow1.*l1pow1))+...
    0.5.*csc(Phi).*(1-cos(2.*Phi)).*((R2.^2.*l1pow1-R1.^2.*l2pow1)./(l2pow1.*l1pow1))-...
    1.5.*sin(2.*Phi).*log((cos(Phi)-R2+l2pow1)./(cos(Phi)-R1+l1pow1))),...
...
KernelVx2=double(KernelVx2);

%%

figure(1);

PP1=plot(Longitude,KernelVx1);
PP1.LineWidth=2.0;
PP1.Color=[1,0,0];
PP1.ColorMode='manual';
hold on;

PP1=plot(Longitude,KernelVx2);
PP1.LineWidth=2.0;
PP1.Color=[0,1,0];
PP1.ColorMode='manual';
hold off;
...
AP=gca;
AP.YLim=[-1.1*max(abs(KernelVx2)),1.1*max(abs(KernelVx2))];

legend('Double-precision computation','Symbolic computation','Location','southeast')

figure(2);

PP1=plot(Longitude,KernelVx1-KernelVx2);
PP1.LineWidth=2.0;
PP1.Color=[0,0,1];
PP1.ColorMode='manual';
hold off;
