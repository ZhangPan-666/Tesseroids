...
KernelVzzz2=double(KernelVzzz2);

%%

figure(1);

PP1=plot(Longitude,KernelVzzz1);
PP1.LineWidth=2.0;
PP1.Color=[1,0,0];
PP1.ColorMode='manual';
hold on;

PP1=plot(Longitude,KernelVzzz2);
PP1.LineWidth=2.0;
PP1.Color=[0,1,0];
PP1.ColorMode='manual';
hold off;
...
AP=gca;
AP.YLim=[-1.1*max(abs(KernelVzzz2)),1.1*max(abs(KernelVzzz2))];

legend('Double-precision computation','Symbolic computation','Location','northeast')

figure(2);

PP1=plot(Longitude,KernelVzzz1-KernelVzzz2);
PP1.LineWidth=2.0;
PP1.Color=[0,0,1];
PP1.ColorMode='manual';
hold off;
