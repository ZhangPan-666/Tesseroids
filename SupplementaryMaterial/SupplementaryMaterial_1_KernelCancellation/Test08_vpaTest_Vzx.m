...
KernelVzx2=double(KernelVzx2);

%%

figure(1);

PP1=plot(Longitude,KernelVzx1);
PP1.LineWidth=2.0;
PP1.Color=[1,0,0];
PP1.ColorMode='manual';
hold on;

PP1=plot(Longitude,KernelVzx2);
PP1.LineWidth=2.0;
PP1.Color=[0,1,0];
PP1.ColorMode='manual';
hold off;
...
AP=gca;
AP.YLim=[-1.1*max(abs(KernelVzx2)),1.1*max(abs(KernelVzx2))];

legend('Double-precision computation','Symbolic computation','Location','northeast')

figure(2);

PP1=plot(Longitude,KernelVzx1-KernelVzx2);
PP1.LineWidth=2.0;
PP1.Color=[0,0,1];
PP1.ColorMode='manual';
hold off;
