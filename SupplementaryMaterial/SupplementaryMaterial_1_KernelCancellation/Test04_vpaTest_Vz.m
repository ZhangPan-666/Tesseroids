...
KernelVz2=double(KernelVz2);

%%

figure(1);

PP1=plot(Longitude,KernelVz1);
PP1.LineWidth=2.0;
PP1.Color=[1,0,0];
PP1.ColorMode='manual';
hold on;

PP1=plot(Longitude,KernelVz2);
PP1.LineWidth=2.0;
PP1.Color=[0,1,0];
PP1.ColorMode='manual';
hold off;
...
AP=gca;
AP.YLim=[-1.1*max(abs(KernelVz2)),1.1*max(abs(KernelVz2))];

legend('Double-precision computation','Symbolic computation','Location','northeast')

figure(2);

PP1=plot(Longitude,KernelVz1-KernelVz2);
PP1.LineWidth=2.0;
PP1.Color=[0,0,1];
PP1.ColorMode='manual';
hold off;
