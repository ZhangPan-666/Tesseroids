...
KernelV2=vpa(0.5.*cosd(FaiI).*( ...
    (3.*cos(Phi)+R2).*l2pow1-(3.*cos(Phi)+R1).*l1pow1+ ...
    (1-3.*cos(Phi).^2).*log((cos(Phi)-R2+l2pow1)./(cos(Phi)-R1+l1pow1))), ...
    GuardDigits);

KernelV2=double(KernelV2);

%%

figure(1);

PP1=plot(Longitude,KernelV1);
PP1.LineWidth=2.0;
PP1.Color=[1,0,0];
PP1.ColorMode='manual';
hold on;

PP1=plot(Longitude,KernelV2);
PP1.LineWidth=2.0;
PP1.Color=[0,1,0];
PP1.ColorMode='manual';
hold off;
...
AP=gca;
AP.YLim=[-1.1*max(abs(KernelV2)),1.1*max(abs(KernelV2))];

legend('Double-precision computation','Symbolic computation','Location','southeast')

figure(2);

PP1=plot(Longitude,KernelV1-KernelV2);
PP1.LineWidth=2.0;
PP1.Color=[0,0,1];
PP1.ColorMode='manual';
hold off;
