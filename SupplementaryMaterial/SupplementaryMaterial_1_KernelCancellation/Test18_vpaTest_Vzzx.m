...
KernelVzzx1=3.*cosd(FaiI).*Tx.*(R2.^4.*(1-R2.*cos(Phi)).*l1pow5-R1.^4.*(1-R1.*cos(Phi)).*l2pow5)./(l2pow5.*l1pow5);
...
KernelVzzx2=vpa(3.*cosd(FaiI).*Tx.*(R2.^4.*(1-R2.*cos(Phi)).*l1pow5-R1.^4.*(1-R1.*cos(Phi)).*l2pow5)./(l2pow5.*l1pow5), ...
    GuardDigits);

KernelVzzx2=double(KernelVzzx2);

%%

figure(1);

PP1=plot(Longitude,KernelVzzx1);
PP1.LineWidth=2.0;
PP1.Color=[1,0,0];
PP1.ColorMode='manual';
hold on;

PP1=plot(Longitude,KernelVzzx2);
PP1.LineWidth=2.0;
PP1.Color=[0,1,0];
PP1.ColorMode='manual';
hold off;
...
AP=gca;
AP.YLim=[-1.1*max(abs(KernelVzzx2)),1.1*max(abs(KernelVzzx2))];

legend('Double-precision computation','Symbolic computation','Location','southeast')

figure(2);

PP1=plot(Longitude,KernelVzzx1-KernelVzzx2);
PP1.LineWidth=2.0;
PP1.Color=[0,0,1];
PP1.ColorMode='manual';
hold off;
