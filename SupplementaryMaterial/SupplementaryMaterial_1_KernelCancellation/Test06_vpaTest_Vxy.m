...
KernelVxy1=cosd(FaiI).*0.5.*sin(2.*Alpha).*(...
    csc(Phi).^2.*(...
...
KernelVxy2=vpa(cosd(FaiI).*0.5.*sin(2.*Alpha).*(...
    csc(Phi).^2.*(...
    cos(Phi).*(-5+3.*cos(Phi).^2).*((l1pow3-l2pow3)./(l2pow3.*l1pow3))+...
    3.*(-1+7.*cos(Phi).^2-4.*cos(Phi).^4).*((R2.*l1pow3-R1.*l2pow3)./(l2pow3.*l1pow3))+...
    6.*cos(Phi).^3.*(-2+cos(2.*Phi)).*((R2.^2.*l1pow3-R1.^2.*l2pow3)./(l2pow3.*l1pow3))+...
...
KernelVxy2=double(KernelVxy2);

%%

figure(1);

PP1=plot(Longitude,KernelVxy1);
PP1.LineWidth=2.0;
PP1.Color=[1,0,0];
PP1.ColorMode='manual';
hold on;

PP1=plot(Longitude,KernelVxy2);
PP1.LineWidth=2.0;
PP1.Color=[0,1,0];
PP1.ColorMode='manual';
hold off;
...
AP=gca;
AP.YLim=[-1.1*max(abs(KernelVxy2)),1.1*max(abs(KernelVxy2))];

legend('Double-precision computation','Symbolic computation','Location','northwest')

figure(2);

PP1=plot(Longitude,KernelVxy1-KernelVxy2);
PP1.LineWidth=2.0;
PP1.Color=[0,0,1];
PP1.ColorMode='manual';
hold off;
